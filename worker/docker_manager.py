...
class DockerManager:
    ...
    def _create_secure_config(self, config: ContainerConfig) -> Dict[str, Any]:
    ...
    async def create_container(self, config: ContainerConfig, container_id: Optional[str] = None) -> str:
        """
        ...
        try:
            docker_config, workspace_volume = self._create_secure_config(config)
            ...
            # Store workspace volume mapping
            container.labels['workspace_volume'] = workspace_volume
            container.labels['grid_x_id'] = container_id
